//! The platform-independent request dispatcher: authentication, the permission gate, and
//! what each request actually does.
//!
//! The message *types* live in `ultidesk-ipc`, so that a client can speak them. This does
//! not: enforcement belongs on the machine providing the capability
//! (docs/permissions.md), and a client that could see the gate would still not be the one
//! running it.
//!
//! Transports live in `pipe.rs`, `unix_socket.rs`, `tcp.rs` and `quic.rs`. Keeping
//! dispatch here — behind an [`Injector`] trait — means the safety-critical logic (auth
//! gating, and releasing all held keys/buttons when a session ends) is unit-tested with a
//! mock injector, without needing a pipe, a GUI, or real input.
...
// The wire types live in `ultidesk-ipc` so that both ends can speak them; what stays here
// is the half a client must not be able to influence — who is allowed to ask for what,
// and what actually happens when they do.
pub use ultidesk_ipc::message::{
