...
///
/// `ULTIDESK_CONFIG_DIR` overrides it, which is what the tests use and what lets a
/// portable install keep its configuration beside the binary.
pub fn config_dir() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("ULTIDESK_CONFIG_DIR") {
        return Some(PathBuf::from(dir));
    }
    #[cfg(windows)]
    {
        std::env::var_os("APPDATA").map(|a| PathBuf::from(a).join("Ultidesk"))
    }
    #[cfg(not(windows))]
    {
        ...
        if let Some(x) = std::env::var_os("XDG_CONFIG_HOME") {
            return Some(PathBuf::from(x).join("ultidesk"));
        }
        std::env::var_os("HOME").map(|h| PathBuf::from(h).join(".config").join("ultidesk"))
    }
}
...
mod tests {
    ...
    fn the_config_directory_can_be_overridden() {
        // Relied on by the tests above and by a portable install.
        let key = "ULTIDESK_CONFIG_DIR";
        let previous = std::env::var_os(key);
        std::env::set_var(key, "/tmp/ultidesk-override-check");
        assert_eq!(
            config_dir(),
            Some(PathBuf::from("/tmp/ultidesk-override-check"))
        );
        match previous {
